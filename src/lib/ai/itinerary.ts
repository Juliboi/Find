...
/**
 * Top-level "plan my whole day" call for the v2 architecture. Sends the
 * free-form day description to the `plan-itinerary` edge function and
 * coerces the result into a typed `Itinerary`. When Supabase/OpenAI is
 * not configured (or the call fails), returns a curated SAMPLE itinerary
 * so the sandbox UI is always explorable — flagged via `usedAi: false`.
 */
